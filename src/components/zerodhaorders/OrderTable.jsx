...
const OrderTable = ({
    ...
    // Separate regular orders and AMO orders
    const regularOrders = orders.filter(order => order.variety === "regular");
    const amoOrders = orders.filter(order => order.variety === "amo");
    ...
    if (!isOpenOrders) {
        return renderOrderTable(orders);
    }

    return (
        <Box>
            {regularOrders.length > 0 && renderOrderTable(regularOrders, "Regular Orders")}
            {regularOrders.length > 0 && amoOrders.length > 0 && <Divider sx={{ my: 2 }} />}
            {amoOrders.length > 0 && (
                <Box sx={{ py: 1, px: 3, bgcolor: 'warning.light', borderRadius: 1, mb: 1 }}>
                    <Typography
                        variant="subtitle2"
                        color="white"
                        sx={{ fontWeight: 700, fontSize: '1rem', letterSpacing: 1 }}
                    >
                        After Market Orders
                    </Typography>
                </Box>
            )}
            {amoOrders.length > 0 && renderOrderTable(amoOrders)}
        </Box>
    );
};
